...
public class MatrixSlider : MonoBehaviour {
    ...
    public void SwitchButtonsStaus(bool buttonFlag)
    {
        for(int i = 0; i < matrixButtons.Length; i++)
        {
            matrixButtons[i].interactable = buttonFlag;
        }

        for(int i = 0; i < matrixResultButtons.Length; i++)
        {
            matrixResultButtons[i].interactable = buttonFlag;
        }
    }
